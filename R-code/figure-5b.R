################################################################################
# INDIVIDUAL-LEVEL ANALYSIS (NO SPECIES AGGREGATION)
# Mirrors species-level code structure for direct comparison
################################################################################
...
# ============================================================================
# 1. LOAD INDIVIDUAL-LEVEL DATA (NO AGGREGATION)
# ============================================================================
...
morpho_final <- trait_clean[tree$tip.label, c("Beak.Length_Culmen", "Wing.Length")]
...
morpho_loadings <- as.data.frame(ppca_result$L[, 1:2])
colnames(morpho_loadings) <- c("PC1", "PC2")
morpho_loadings$Trait <- c("Beak Length", "Wing Length")
...
tree_dat_with_pc <- trait_clean %>%
  select(Species, Beak.Length_Culmen, Wing.Length) %>% 
...
beak_stats <- tree_dat_with_pc %>%
  summarise(
    N = n(),
    Mean_beak = mean(Beak.Length_Culmen, na.rm = TRUE),
    Median_beak = median(Beak.Length_Culmen, na.rm = TRUE),
    Q25 = quantile(Beak.Length_Culmen, 0.25, na.rm = TRUE),
    Q75 = quantile(Beak.Length_Culmen, 0.75, na.rm = TRUE),
    Min = min(Beak.Length_Culmen, na.rm = TRUE),
    Max = max(Beak.Length_Culmen, na.rm = TRUE),
    `Prop <=31mm` = mean(Beak.Length_Culmen <= 31, na.rm = TRUE)
  )
print(beak_stats) #------------------------------------------------------------- 1) beak_stats
...
beak_check <- tree_dat_with_pc %>%
  filter(Species %in% high_pc1_species_clean) %>%
  select(Species, Beak.Length_Culmen, logMP, PC1) %>%
  arrange(desc(PC1))
print(beak_check) #------------------------------------------------------------- 2) beak check

# 4. Decision Tree Birds
cat("\n4. DECISION TREE'S '>=31mm' BIRDS IN PC1 SPACE:\n")
tree_threshold_birds <- tree_dat_with_pc %>%
  filter(Beak.Length_Culmen <= 31) %>%
  select(Species, Beak.Length_Culmen, logMP, PC1) %>%
  arrange(desc(logMP)) %>%
  head(10)
...
# 5. Correlation
cat("\n5. CORRELATION: Beak Length vs PC1:\n")
cor_test <- cor.test(tree_dat_with_pc$Beak.Length_Culmen, tree_dat_with_pc$PC1, use = "complete.obs")
print(cor_test) #--------------------------------------------------------------- 4) cor_test


# 7. Statistical test
cat("\n7. STATISTICAL COMPARISON:\n")
model1 <- lm(logMP ~ Beak.Length_Culmen, data = tree_dat_with_pc)
model2 <- lm(logMP ~ PC1, data = tree_dat_with_pc)
model3 <- lm(logMP ~ Beak.Length_Culmen + PC1, data = tree_dat_with_pc)
...
print(simple_tree)
rpart.plot(simple_tree, main = "Current Decision Tree")

# Save results for discussion
diagnostic_results <- list(
  beak_stats = beak_stats,
  high_pc1_beaks = beak_check,
  tree_threshold_birds = tree_threshold_birds,
  correlation = cor_test,
  models = list(beak_only = model1, pc1_only = model2, combined = model3)
)
...
# Panel 1: Beak vs PC1
p1 <- ggplot(tree_dat_with_pc, aes(x = Beak.Length_Culmen, y = PC1)) +
  geom_point(aes(color = logMP), size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", color = "red", se = FALSE, linetype = "dashed") +
  geom_vline(xintercept = 31, linetype = "dotted", color = "blue", linewidth = 1) +
  scale_color_viridis_c(option = "plasma") +
  labs(title = "Beak Length vs PC1", x = "Beak Length (mm)", y = "PC1 Score") +
  theme_minimal()

# Panel 2: The U-Shape Pattern (Quadratic Fit)
p2 <- ggplot(tree_dat_with_pc, aes(x = Beak.Length_Culmen, y = logMP)) +
  ...
  labs(title = "U-Shaped Accumulation Pattern", x = "Beak Length (mm)", y = "log(MP+1)") +
  theme_minimal()

# Panel 3: PC1 vs logMP
p3 <- ggplot(tree_dat_with_pc, aes(x = PC1, y = logMP)) +
  geom_point(aes(color = Beak.Length_Culmen <= 31), size = 3, alpha = 0.7) +
  geom_smooth(method = "lm", color = "black", se = FALSE) +
  scale_color_manual(values = c("gray", "darkblue"), name = "Beak <=31mm") +
...
m_lin <- lm(logMP ~ Beak.Length_Culmen, data = tree_dat_with_pc)
m_quad <- lm(logMP ~ Beak.Length_Culmen + I(Beak.Length_Culmen^2), data = tree_dat_with_pc)
...
m_gam <- gam(
  logMP ~ s(Beak.Length_Culmen, k = 4),
...
model_linear <- lm(logMP ~ Beak.Length_Culmen, data = tree_dat_with_pc)
model_quad   <- lm(logMP ~ poly(Beak.Length_Culmen, 2), data = tree_dat_with_pc) # The U-shape
